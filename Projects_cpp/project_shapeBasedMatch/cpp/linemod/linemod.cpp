...
// This code is also subject to the license terms in the LICENSE_WillowGarage.md
// file found in this module's directory
...
void QuantizedPyramid::selectScatteredFeatures(
  ...
  while (features.size() < num_features) {
    ...
    for (int j = 0; (j < (int)features.size()) && keep; ++j) {
      ...
      keep = (c.f.x - f.x) * (c.f.x - f.x) + (c.f.y - f.y) * (c.f.y - f.y) >=
             distance_sq;
    }
    if (keep)
...
void drawFeatures(InputOutputArray img, const std::vector<Template> &templates,
  ...
  for (const Template &t : templates) {
    ...
    for (const Feature &f : t.features) {
      drawMarker(img, tl + Point(f.x, f.y), colors[int(modality != 0)],
 ...
 * \param      threshold Magnitude threshold. Keep only gradients whose norms
 * are larger than this.
 */
static void quantizedOrientations(const Mat &src, Mat &magnitude, Mat &angle,
                                  float threshold) {
  ...
  // For some reason cvSmooth/cv::GaussianBlur, cvSobel/cv::Sobel have different
  // defaults for border handling...
...
public:
  ColorGradientPyramid(const Mat &src, const Mat &mask, float weak_threshold,
                       size_t num_features, float strong_threshold);
...
bool ColorGradientPyramid::extractTemplate(Template &templ) const {
  ...
  // Use heuristic based on surplus of candidates in narrow outline for initial
  // distance threshold
...
ColorGradient::ColorGradient()
    : weak_threshold(10.0f), num_features(63), strong_threshold(55.0f) {}
...
Ptr<ColorGradient> ColorGradient::create(float weak_threshold,
                                         size_t num_features,
                                         float strong_threshold) {
...
DepthNormalPyramid::DepthNormalPyramid(const Mat &src, const Mat &_mask,
    ...
    : mask(_mask), pyramid_level(0), num_features(_num_features),
...
bool DepthNormalPyramid::extractTemplate(Template &templ) const {
    ...
    for (int c = 0; c < normal.cols; ++c) {
      if (no_mask || mask_r[c]) {
        ...
        if (quantized != 0 && quantized != 255) // background and shadow
        {
          ...
          // Accept if distance to a pixel belonging to a different label is
          // greater than some threshold. IOW, ideal feature is in the center of
          // a large homogeneous region.
  ...
  // Prefer large distances, but also want to collect features over all 8
  // labels. So penalize labels with lots of candidates.
  ...
  float area =
      no_mask ? (float)normal.total() : (float)countNonZero(local_mask);
...
DepthNormal::DepthNormal()
    : distance_threshold(2000), difference_threshold(50), num_features(63),
      extract_threshold(2) {}

DepthNormal::DepthNormal(int _distance_threshold, int _difference_threshold,
                         size_t _num_features, int _extract_threshold)
    : distance_threshold(_distance_threshold),
      difference_threshold(_difference_threshold), num_features(_num_features),
      extract_threshold(_extract_threshold) {}

Ptr<DepthNormal> DepthNormal::create(int distance_threshold,
                                     int difference_threshold,
                                     size_t num_features,
                                     int extract_threshold) {
  return makePtr<DepthNormal>(distance_threshold, difference_threshold,
...
Ptr<QuantizedPyramid> DepthNormal::processImpl(const Mat &src,
  ...
  return makePtr<DepthNormalPyramid>(src, mask, distance_threshold,
                                     difference_threshold, num_features,
                                     extract_threshold);
}
...
static void orUnaligned8u(const uchar *src, const int src_stride, uchar *dst,
  ...
  for (int r = 0; r < height; ++r) {
    ...
    // Use LDDQU for fast unaligned load
      ...
      for (; c < width - 15; c += 16) {
        __m128i val =
            _mm_lddqu_si128(reinterpret_cast<const __m128i *>(src + c));
        __m128i *dst_ptr = reinterpret_cast<__m128i *>(dst + c);
    ...
    // Fall back to MOVDQU
      ...
      for (; c < width - 15; c += 16) {
        __m128i val =
            _mm_loadu_si128(reinterpret_cast<const __m128i *>(src + c));
        __m128i *dst_ptr = reinterpret_cast<__m128i *>(dst + c);
        *dst_ptr = _mm_or_si128(*dst_ptr, val);
      }
    }
...
static void spread(const Mat &src, Mat &dst, int T) {
  ...
  for (int r = 0; r < T; ++r) {
    ...
    for (int c = 0; c < T; ++c) {
      orUnaligned8u(&src.at<unsigned char>(r, c), static_cast<int>(src.step1()),
                    dst.ptr(), static_cast<int>(dst.step1()), src.cols - c,
                    height);
    }
  }
}
 ...
 */
static void computeResponseMaps(const Mat &src,
 ...
 * \param[out] linearized   The response map in linearized order. It has T*T
 * rows, each of which is a linear memory of length (W/T)*(H/T). \param      T
 * Sampling step.
 */
...
accessLinearMemory(const std::vector<Mat> &linear_memories, const Feature &f,
                   int T, int W) {
...
/**
 * \brief Compute similarity measure for a given template at each sampled image
 * location.
 *
 * Uses linear memories to compute the similarity measure as described in
 * Fig. 7.
 *
 * \param[in]  linear_memories Vector of 8 linear memories, one for each label.
 * \param[in]  templ           Template to match against.
 * \param[out] dst             Destination 8-bit similarity image of size (W/T,
 * H/T). \param      size            Size (W, H) of the original input image.
 * \param      T               Sampling step.
 */
static void similarity(const std::vector<Mat> &linear_memories,
                       const Template &templ, Mat &dst, Size size, int T) {
  // 63 features or less is a special case because the max similarity
  // per-feature is 4. 255/4 = 63, so up to that many we can add up similarities
  // in 8 bits without worrying about overflow. Therefore here we use
  // _mm_add_epi8 as the workhorse, whereas a more general function would use
  // _mm_add_epi16.
...
#if CV_SSE3
    if (haveSSE3) {
      // LDDQU may be more efficient than MOVDQU for unaligned load of next 16
      // responses
      for (; j < template_positions - 15; j += 16) {
        __m128i responses =
            _mm_lddqu_si128(reinterpret_cast<const __m128i *>(lm_ptr + j));
      ...
      for (; j < template_positions - 15; j += 16) {
        __m128i responses =
            _mm_loadu_si128(reinterpret_cast<const __m128i *>(lm_ptr + j));
        __m128i *dst_ptr_sse = reinterpret_cast<__m128i *>(dst_ptr + j);
        *dst_ptr_sse = _mm_add_epi8(*dst_ptr_sse, responses);
      }
    }
 ...
 */
static void similarityLocal(const std::vector<Mat> &linear_memories,
                            const Template &templ, Mat &dst, Size size, int T,
                            Point center) {
...
#if CV_SSE3
    if (haveSSE3) {
      // LDDQU may be more efficient than MOVDQU for unaligned load of 16
      // responses from current row
      for (int row = 0; row < 16; ++row) {
        __m128i aligned =
            _mm_lddqu_si128(reinterpret_cast<const __m128i *>(lm_ptr));
      ...
      for (int row = 0; row < 16; ++row) {
        __m128i aligned =
            _mm_loadu_si128(reinterpret_cast<const __m128i *>(lm_ptr));
        dst_ptr_sse[row] = _mm_add_epi8(dst_ptr_sse[row], aligned);
        lm_ptr += W; // Step to next row
      }
    } else
...
static void addUnaligned8u16u(const uchar *src1, const uchar *src2, ushort *res,
                              int length) {
...
void Detector::match(const std::vector<Mat> &sources, float threshold,
  ...
  for (int l = 0; l < pyramid_levels; ++l) {
    ...
    for (int i = 0; i < (int)quantizers.size(); ++i) {
      ...
      if (quantized_images
              .needed()) // use copyTo here to side step reference semantics.
  ...
  // Sort matches by similarity, and prune any duplicates introduced by pyramid
  // refinement
...
void Detector::matchClass(
    const LinearMemoryPyramid &lm_pyramid, const std::vector<Size> &sizes,
    float threshold, std::vector<Match> &matches, const String &class_id,
  ...
  for (size_t template_id = 0; template_id < template_pyramids.size();
    ...
    // Convert user-friendly percentage to raw similarity threshold. The
    // percentage threshold scales from half the max response (what you would
    // expect from applying the template to a completely random image) to the
    // max response. NOTE: This assumes max per-feature response is 4, so we
    // scale between [2*nf, 4*nf].
    int raw_threshold = static_cast<int>(
        2 * num_features + (threshold / 100.f) * (2 * num_features) + 0.5f);
...
int Detector::numTemplates() const {
  int ret = 0;
  TemplatesMap::const_iterator i = class_templates.begin(),
                               iend = class_templates.end();
  for (; i != iend; ++i)
...
String Detector::readClass(const FileNode &fn,
  ...
  for (; tps_it != tps_it_end; ++tps_it, ++expected_id) {
    ...
    FileNodeIterator templ_it = templates_fn.begin(),
...
void Detector::writeClass(const String &class_id, FileStorage &fs) const {
  ...
  for (size_t i = 0; i < tps.size(); ++i) {
    ...
    fs << "{";
    fs << "template_id"
       << int(i); // TODO is this cast correct? won't be good if rolls over...
    fs << "templates"
       << "[";
