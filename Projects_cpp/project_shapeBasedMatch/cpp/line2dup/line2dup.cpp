...
// void Gradient::read(FileNode& fn) {
//   x = fn["x"];
//   y = fn["y"];
//   label = fn["label"];
//   angle = fn["angle"];
...
//   fs << "{" << "x" << x << "y" << y << "label" << label << "angle" << angle
//   << "}";
// }

/// class ShapeTemplate

// void ShapeTemplate::read(FileNode &fn) {
...
//   int i = 0;
//   for (auto f_feature : featrues_fn) {
...
void line2Dup::ShapeTemplate::show_in(Mat &background,
...
void TemplateSearch::build(const Search &search, ShapeTemplate &base) {
  ...
  //                       background.type());
  // warpAffine(background, rotate_background, rotate_mat,
  //                rotate_background.size(), INTER_LINEAR,
  //                BORDER_REPLICATE);
...
// void line2Dup::Range::read(FileNode &fn) {
...
// void line2Dup::Range::write(FileStorage &fs) const {
//   fs << "{"
...
// void TemplateSearch::read(FileNode &fn) {
//   fn["rows"] >> rows;
//   fn["cols"] >> cols;

//   fn["scale_range"] >> region.scale;
//   fn["angle_range"] >> region.angle;
...
//   fs << "rows" << rows;
//   fs << "cols" << cols;
//   fs << "scale_range" << region.scale;
//   fs << "angle_range" << region.angle;
...
// void ColorGradientPyramid::read(FileNode &fn) {
...
//      << "count_kernel_size" << count_kernel_size
...
// void LinearMemory::read(FileNode &fn) {
...
// void Detector::read(FileNode& fn) {
//   fn["name"] >> name;
...
//     fs << templates_pyramid[i];
//   }
//   fs << "]";

//   fs << "memories_pyramid" << "[";
//   for (int i = 0; i < (int)memories_pyramid.size(); i++) {
//     fs << memories_pyramid[i];
//   }
//   fs << "]";
// }