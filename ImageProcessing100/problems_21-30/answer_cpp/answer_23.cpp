...
Mat histEqualize(Mat &I, double M0 = 128.0, double S0 = 52.0) {
  CV_Assert(I.type() == CV_8UC3);
  ...
  int h_tolnum = H.getNum();

  for(int i = 0; i < n_rows; i++)
    for(int j = 0; j < n_cols; j++)
      for(int c = 0; c < n_channel; c++) {
        double _pijc = I.at<Vec3b>(i, j)[c];
        _pijc = 255.0 / h_tolnum * H.getSum2z(_pijc);
...
int main() {
  ...
  // Hist.showHist(2, 360);
  Mat A = histEqualize(img);
  imshow("histEqualize", A);
