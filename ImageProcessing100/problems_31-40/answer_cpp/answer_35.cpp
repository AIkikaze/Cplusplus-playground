 ...
 * @Date: 2023-05-08 14:53:37
 * @LastEditors: AIkikaze wenwenziy@163.com
 * @LastEditTime: 2023-05-09 15:47:43
 * @FilePath: \Cplusplus-playground\ImageProcessing100\problems_31-40\answer_cpp\answer_35.cpp
 * @Description: 
 * 
 */
...
void rangepassFilter(Mat &I, double R[]) {
  // 中心化经过傅里叶变换的复数矩阵
  recenter(I);
  // 初始化高通滤波核
  double cy = (double)I.rows/2.0;
  double cx = (double)I.cols/2.0;
  double r_filter_min = sqrt(cy*cy + cx*cx) * R[0];
  double r_filter_max = sqrt(cy*cy + cx*cx) * R[1];
  for(int y = 0; y < I.rows; y++) {
    for(int x = 0; x < I.cols; x++) {
      double dist = sqrt(pow((double)y-cy, 2) + pow((double)x-cx, 2));
      if(dist < r_filter_min || dist > r_filter_max) {
...
int main() {
  Mat img = imread("../imagelib/test.png", IMREAD_GRAYSCALE);
  
  preinit(img);
  Mat F = myDft(img);
  double range[] = { 0.1, 0.5 };
  rangepassFilter(F, range);
  Mat iF = myIdft(F);
